  ...
  methods: {
    ...
    choose (option) {
      ...
      if (this.currentStepType === 'button') {
        if (option.action === 'submit') {
          ...
          // console.log('VALUE OF BUTTON: ', output, this.step, )
          if (output == 'Yup') {this.res = this.sumAllRes()}
      ...
      if (increaseStep) {
        ...
        // console.log('VALUE OF SELECTION: ', option.text, this.step, this.screenplay[this.step].name)
        if (option.text == 'Yup') {
          this.res = this.sumAllRes()
          this.register(this.res)
        }
    ...
    sumAllRes() {
      ...
      for (var i=0; i < this.screenplay.length; i++) {
        if (dict_keys.indexOf(this.screenplay[i].name) != -1) {
          resDict[this.screenplay[i].name] = this.screenplay[i].answer
        }
      }
      return resDict      
    },
